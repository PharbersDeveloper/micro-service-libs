...
//        log.info("batchSize" + batchSize);
    ...
    private Map<String, Long> getStreamOffset() throws IOException {
        Map<String, Object> offset = context.offsetStorageReader().offset(Collections.singletonMap(JOBID_FIELD, traceID));
        if (offset != null) {
            Object lastRecordedOffset = offset.get(POSITION_FIELD);
            if (lastRecordedOffset != null && !(lastRecordedOffset instanceof String))
                throw new ConnectException("Offset position is the incorrect type");
            streamOffset = (lastRecordedOffset != null) ? recodeOffset((String) lastRecordedOffset) : new HashMap<>();
        } else {
            streamOffset = new HashMap<>();
        }
