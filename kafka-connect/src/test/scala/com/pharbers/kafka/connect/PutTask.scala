...
object PutTask extends App {
    ...
    val iter = assets.find(Document.parse("{'dfs':[]}, {'isNewVersion':true}")).iterator()
    ...
    def getTask(file: BsonDocument, assset: BsonDocument, jobId: String): OssTask ={
        val traceId = assset.getString("traceId").getValue
        ...
        new OssTask(assetId, jobId, traceId, ossKey, new util.ArrayList[Integer](), fileType, fileName, "",
