...
object HTTP {
	def apply(url : String) : httpOpt = httpOpt(url)
}

case class httpOpt(url : String) {
	...
	private def encodePostParameters(data: Map[String, String]): immutable.Iterable[String] =
		for ((name,value) <- data)
	...
	def get(parameters : Map[String, String]): httpOpt = {
		loadCookies
		connection.connect()
		storeCookies
